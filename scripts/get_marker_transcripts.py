...
def fit_glsar_with_splines(transcript_data):
    try:
        ...
        return {
            "query_id": query_id,
            "DIC_coef": result.params["DIC_std"],
            "DIC_pval": result.pvalues["DIC_std"],
            "AIC": result.aic
        }
...
if __name__ == "__main__":
    ...
    DIC['Date.Time'] = pd.to_datetime(DIC['Date.Time'])
    meta['Time'] = (meta['Date'] - meta['Date'].min()).dt.total_seconds() / 3600  # Hours since start
    DIC['Time'] = (DIC['Date.Time'] - meta['Date'].min()).dt.total_seconds() / 3600

    # Interpolate DIC values for meta timestamps
    DIC = DIC.set_index('Date.Time').reindex(meta['Date']).interpolate(method='time').reset_index()
    meta = pd.merge(meta, DIC[['Time', 'y']], left_on='Time', right_on='Time', how='left')
