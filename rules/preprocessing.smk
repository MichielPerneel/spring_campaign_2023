...
rule bbmap:
        ...
        log: os.path.join(config['log_dir'], 'bbmap', '{sample}.log'),
        params:
            spikefile = config['spikefile']
        threads: 4
        shell:'''
            module load BBMap
            bbmap.sh in1={input.r1} in2={input.r2} ref={params.spikefile} outu1={output.r1} outu2={output.r2} > {log} 2>&1
            '''

rule trimmomatic_combined:
        '''
        This step removes sequencing adapters from the combined set of sequences from run 1 and 2 using Trimmomatic. 
        '''
        input:
                r1 = os.path.join(config['combined_raw_reads'], '{sample}_combined_R1.fastq.gz'),
                r2 = os.path.join(config['combined_raw_reads'], '{sample}_combined_R2.fastq.gz'),
        output:
                r1 = os.path.join(config['scratch_dir'], 'trimmed',
                                'combined' , '{sample}_1.trimmed.fastq.gz'),
                r2 = os.path.join(config['scratch_dir'], 'trimmed',
                                'combined' , '{sample}_2.trimmed.fastq.gz'),
                # reads where trimming entirely removed the mate
                r1_unpaired = os.path.join(config['scratch_dir'], 'trimmed',
                                'combined' , '{sample}_1.unpaired.fastq.gz'),
                r2_unpaired = os.path.join(config['scratch_dir'], 'trimmed',
                                'combined' , '{sample}_2.unpaired.fastq.gz')
        params:
                trimmer=['ILLUMINACLIP:{}:2:30:7'.format(config['adapters']), 
                        'LEADING:2', 'TRAILING:2', 'SLIDINGWINDOW:4:2', 'MINLEN:50']
        shell:'''
...
rule combined_sample_rRNA_cleanup:
        '''
        This step removes ribosomal RNA reads using ribodetector.
        '''
        input:
        ...
        output:
            r1 = os.path.join(config['scratch_dir'], 'cleanup',
                            'combined' , '{sample}_1.rRNA_removed.fastq.gz'),
            r2 = os.path.join(config['scratch_dir'], 'cleanup',
                            'combined' , '{sample}_2.rRNA_removed.fastq.gz'),
            rna1 = os.path.join(config['scratch_dir'], 'rRNA',
                            'combined' , '{sample}_1.rRNA.fastq.gz'),
            rna2 = os.path.join(config['scratch_dir'], 'rRNA',
                            'combined' , '{sample}_2.rRNA.fastq.gz')
        log: os.path.join(config['log_dir'], 'ribodetector', 'combined', '{sample}.log')
        conda: os.path.join(config['conda_environments'], 'ribodetector.yaml')
        shell:'''
            ribodetector_cpu -t 20 \
                -i {input.r1} {input.r2} \
...
rule bbmap_combined:
        ...
        log: os.path.join(config['log_dir'], 'bbmap', 'combined', '{sample}.log')
        params:
            spikefile = config['spikefile']
        threads: 4
        shell:'''
